...
{
  // 이 함수는 Qt GUI 스레드(메인 스레드)에서 호출된다.
  cv::Mat frame;
  {
    std::lock_guard<std::mutex> lock(frame_mutex_);
    if (!has_frame_) {
...
int main(int argc, char ** argv)
{
  ...
  // rclcpp::spin()은 블로킹 호출이라 Qt 이벤트 루프와 분리된 스레드에서 돌린다.
  std::thread ros_thread([&node]() {
    rclcpp::spin(node);
  });

  const int result = app.exec();  // Qt 이벤트 루프 (메인 스레드)
