...
void MainWindow::flashButton(QObject *obj)
{
    QAbstractButton *btn = qobject_cast<QAbstractButton*>(obj);
    if (!btn) return;

    // 이미 반짝이는 중이 아닐 때만 원래 스타일을 저장해둡니다.
    // (연타할 때 반짝이는 스타일이 "원래 스타일"로 잘못 저장되는 것을 방지)
    if (!btn->property("flashing").toBool()) {
        btn->setProperty("originalStyle", btn->styleSheet());
    }
    btn->setProperty("flashing", true);

    btn->setStyleSheet(btn->property("originalStyle").toString() + "background-color: #ffe066;");

    QTimer::singleShot(200, this, [btn]() {
        btn->setStyleSheet(btn->property("originalStyle").toString());
        btn->setProperty("flashing", false);
    });
}
...
void MainWindow::udp_read() {
    ...
    while (text_socket->hasPendingDatagrams()) {
        ...
        // UI에 상대방 정보와 메시지 출력
        ui->plainTextEdit_2->appendPlainText("[상대]");
        ui->plainTextEdit_2->appendPlainText(QString("%1")
                                 .arg(message));
    }
}
...
void MainWindow::on_abc_clicked()
{
    ...
    // 화면 출력 (확정 문장 + 실시간 글자)
    ui->plainTextEdit->setPlainText(confirmed_text + current_char);
...
void MainWindow::on_EK_2_clicked()
{
    flashButton(sender());
    ui->stackedWidget->setCurrentIndex(2);
}
...
void MainWindow::on_space_clicked()
{
    ...
    // 화면 출력
    ui->plainTextEdit->setPlainText(confirmed_text);
...
void MainWindow::on_l_clicked() // 모음 버튼: ㅣ (모음 그룹 번호 25번 통일)
{
    ...
    ui->plainTextEdit->setPlainText(confirmed_text + combineHangeul(current_char, current_vowel));
    last_button = 25;
    timer.restart();
}
...
{
    flashButton(sender());
    QString current_display = ui->plainTextEdit->toPlainText();

    if (!current_display.isEmpty()) {
        current_display.chop(1);

        ui->plainTextEdit->setPlainText(current_display);
    }
